...
class Intents {
  ...
  #adoptStair(document, data, plan, entry, intent) {
    ...
    document.updateSource(stairCreateData(entry.level, target.level, data.shapes ?? [], movementActionKeys()));
    return true;
  }
  ...
  async onCreateRegion(document, options, userId) {
    ...
    if (!lower) return;
    const targets = mirrorTargets(lower, INTENTS.STAIR);
    const updates = targets.map((t) => holeAppendData(t.surface, Array.from(document.shapes), {}));
    await applyHoleUpdates(canvas.scene, updates);
  }
}
